...
namespace Pulse.Services.Implementations
{
    public class ActiveDirectoryService : IActiveDirectoryService
    {
        ...
        private readonly string[] ResultFields = new string[]  {
                                ...
                                "division",
                                "employeeid",
                                "st-jobfunctiondescription",
                                "thumbnailphoto",
                                "st-costcenter",
                                "st-costcenterdescription",
                                "telephonenumber",
                                "st-region",
        ...
        public ICollection<ActiveDirectoryUser> SearchUsers(string searchTerm)
        {
            ...
            var oResults = default(SearchResultCollection);
            var RetArray = new ArrayList();



            var returnvalue = new Collection<ActiveDirectoryUser>();

            try
            {
                var _with1 = oSearcher;

                _with1.SearchRoot = new DirectoryEntry(string.Format("LDAP://{0}/{1}",
                    _ldapServer,
                    _ldapContainer
                    ),
                    "",
                ...
                _with1.PropertiesToLoad.AddRange(ResultFields);

                if (searchTerm != null)
                    _with1.Filter = "(|(cn=" + searchTerm.ToLower() + "*)(sn=" + searchTerm.ToLower() + "*)(mail=" + searchTerm.ToLower() + "*)(uid=" + searchTerm.ToLower() + "))";
                else
                    _with1.Filter = "";

                oResults = _with1.FindAll();
                ...
                if (oResults.Count > 0)
                {
                    ...
                    foreach (SearchResult result in oResults)
                    {
                        ...
                        if (result.Properties.Contains("sn"))
                            _lastName = result.Properties["sn"][0].ToString();
                        else _lastName = "";

                        if (result.Properties.Contains("employeeNumber"))
                            _employeeNumber = result.Properties["employeeNumber"][0].ToString();
                        else if (result.Properties.Contains("st-eduid"))
                            _employeeNumber = result.Properties["st-eduid"][0].ToString();
                        else _employeeNumber = "";

                        if (result.Properties.Contains("uid"))
                            _username = result.Properties["uid"][0].ToString();
                        else _username = "";
                        ...
                        var account = new ActiveDirectoryUser
                        {
                            FirstName = _firstName,
                            LastName = _lastName,
                            EmployeeId = _employeeNumber,
                            Username = _username,
                            Email = _email
                        };
        ...
        public ICollection<ActiveDirectoryUser> SearchUsersFull(string searchTerm)
        {
            ...
            try
            {
                using (var searcher = CreateDirectorySearcher(_ldapServer, _ldapContainer, ResultFields))
                {
        ...
        {
            return new DirectorySearcher(new DirectoryEntry($"LDAP://{ldapServer}/{ldapContainer}", null, null, AuthenticationTypes.Anonymous));
        }
        private DirectorySearcher CreateDirectorySearcher(string ldapServer, string ldapContainer, string[] propertiesToLoad)
        {
            var searcher = new DirectorySearcher(new DirectoryEntry($"LDAP://{ldapServer}/{ldapContainer}", null, null, AuthenticationTypes.Anonymous));
        ...
        private ActiveDirectoryUser MapSearchResultToAdUser(SearchResult result)
        {
            return new ActiveDirectoryUser
            {
                ...
                Username = GetProperty(result, "uid"),
