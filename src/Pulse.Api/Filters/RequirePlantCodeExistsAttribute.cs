...
{
    public class RequirePlantCodeExistsAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            if (actionContext.ActionArguments.ContainsKey("code"))
            {
                ...
                var plantService = requestScope?.Resolve<IPlantService>();
                // Synchronously check if the plant exists (for demo; use async if possible)
                var plant = plantService?.GetPlantByCode(code);
