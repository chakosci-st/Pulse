...
{
    public class RequireModuleCodeExistsAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            ...
            if (actionContext.ActionArguments.ContainsKey("code"))
            {
                ...
                var ModuleService = requestScope?.Resolve<IModuleService>();
                // Synchronously check if the plant exists (for demo; use async if possible)
                var plant = ModuleService?.GetModuleByCode(code);

                if (plant == null)
                {
                    actionContext.Response = actionContext.Request.CreateErrorResponse(
                        HttpStatusCode.NotFound, "Plant code does not exist.");
                    return;
                }
            }
