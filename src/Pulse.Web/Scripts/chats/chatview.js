...
(function () {
    ...
    // State
    let rooms = [];         // { room, roomName, roomIcon, roomColor, unreadCount?, lastMessagePreview? }
    let filteredRooms = [];
    ...
    function resizeChatMessages() {
        ...
        const messagesRect = chatMessagesEl.getBoundingClientRect();
        const inputHeight = document.querySelector('.chat-main-input')?.offsetHeight || 0;
        const typingHeight = (typingIndicatorEl && typingIndicatorEl.style.display !== 'none')
            ? typingIndicatorEl.offsetHeight
            : 0;

        const availableHeight = Math.floor(viewportHeight - messagesRect.top - inputHeight - typingHeight - 20);
        if (availableHeight > 200) {
            chatMessagesEl.style.height = `${availableHeight}px`;
            chatMessagesEl.style.maxHeight = `${availableHeight}px`;
        }
    }

    function resizeChatAppContainer() {
        if (!chatAppEl) return;

        const viewportHeight = window.innerHeight || document.documentElement.clientHeight;
        const chatAppRect = chatAppEl.getBoundingClientRect();
        const availableHeight = Math.floor(viewportHeight - chatAppRect.top - 16);

        if (availableHeight > 360) {
            chatAppEl.style.height = `${availableHeight}px`;
            chatAppEl.style.maxHeight = `${availableHeight}px`;
        }
    }
    ...
    function renderRooms(list) {
        ...
        list.forEach(r => {
            ...
            item.setAttribute('data-room-id', r.room);   // NOTE: uses r.room
    ...
    function loadRooms() {
        $.getJSON(window.getApiRootPath() + '/api/chat/rooms', function (data) {
            // Expecting array of: { room, roomName, roomIcon, roomColor, unreadCount?, lastMessagePreview? }
            rooms = data || [];
    ...
    if (roomSearchEl) {
        roomSearchEl.addEventListener('input', function () {
            ...
            filteredRooms = rooms.filter(r =>
                (r.roomName || r.room || '').toLowerCase().includes(term)  // <-- fixed
            );
    ...
    if (roomListEl) {
        roomListEl.addEventListener('click', function (e) {
            ...
            const roomMeta = rooms.find(r => r.room === roomId) || { room: roomId }; // <-- fixed
    ...
    function renderReplyHtml(options) {
        const { sender, alias, senderId, timestamp, messageHtml, isYou } = options;
    ...
    function updateRoomHeader() {
        ...
        const name = currentRoomMeta ?.roomName || currentRoomId;
        const iconClass = currentRoomMeta ?.roomIcon || 'bi bi-chat-dots-fill';
        const color = currentRoomMeta ?.roomColor || '#0d6efd';
    ...
    hub.client.receiveMessage = function (message) {
        ...
        const r = rooms.find(x => x.room === room);   // <-- fixed
        if (r) {
            r.lastMessagePreview = message.Message;   // <-- lowercase
            if (room !== currentRoomId) {
    ...
    hub.client.setRoomMeta = function (meta) {
        currentRoomMeta = {
            room: meta.ProjectNo || meta.room || currentRoomId,
            roomName: meta.RoomName || meta.roomName,
            roomIcon: meta.RoomIcon || meta.roomIcon,
            roomColor: meta.RoomColor || meta.roomColor
        };
        updateRoomHeader();
