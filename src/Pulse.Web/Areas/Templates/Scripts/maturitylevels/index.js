...
$(document).ready(function () {
    var table = $('#maturitylevelsTable').DataTable({
        "processing": true, "responsive": true,
        "serverSide": true,
        "ajax": {
            "url": getApiRootPath() + "/api/maturityLevels/datatables",
            "type": "POST",
        ...
        "initComplete": function () {
            ...
            $tableContainer.find('.dt-paging').appendTo('.card-tools-maturitylevelsTable-pagination');
            $tableContainer.find('.dt-search').appendTo('.card-tools-maturitylevelsTable-filter');
            $tableContainer.find('.dt-length').appendTo('.card-tools-maturitylevelsTable-length');
            $tableContainer.find('.dt-info').appendTo('.card-tools-maturitylevelsTable-size');
          ...
          <div class="flex-fill pr-lg-3">
            <div class="d-flex flex-wrap align-items-center mb-1">
              ${status}
                ...
                <strong>${data.maturityCode}</strong> (${data.maturityNumber})
             
            </div>
            <div class="small text-muted mb-1">
              Sequence: <strong>${data.sequenceNo}</strong>
            </div>
            <div class="small text-muted mb-1">
...
<a title="Display" href="/templates/maturityLevels/display/${data.maturityCode}"
                class="btn btn-outline-secondary rounded-circle d-inline-flex align-items-center justify-content-center p-0"
                style="width: 40px; height: 40px;">
    <i class="bi bi-eye"></i>
</a>
         
<a title="Config" href="/templates/maturityLevels/edit/${data.maturityCode}"
