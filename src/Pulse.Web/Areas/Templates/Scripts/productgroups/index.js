...
$(document).ready(function () {
    var table = $('#productgroupsTable').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": getApiRootPath() + "/api/productGroups/datatables",
            "type": "POST",
        ...
        "initComplete": function () {
            ...
            $tableContainer.find('.dt-paging').appendTo('.card-tools-productgroupsTable-pagination');
            $tableContainer.find('.dt-search').appendTo('.card-tools-productgroupsTable-filter');
            $tableContainer.find('.dt-length').appendTo('.card-tools-productgroupsTable-length');
            $tableContainer.find('.dt-info').appendTo('.card-tools-productgroupsTable-size');
          ...
          <div class="flex-fill pr-lg-3">
            <div class="d-flex flex-wrap align-items-center mb-1">
              ${status}
                ...
                <strong>${data.productGroupName}</strong> (${data.productGroupCode})
             
            </div>
            <div class="small text-muted mb-1">
            Description: ${data.productGroupDescription ?? ''}
            </div>
            <div class="small text-muted mb-1">
...
<a title="Display" href="/templates/productGroups/display/${data.productGroupCode}"
                class="btn btn-outline-secondary rounded-circle d-inline-flex align-items-center justify-content-center p-0"
                style="width: 40px; height: 40px;">
    <i class="bi bi-eye"></i>
</a>
             
<a title="Config" href="/templates/productGroups/edit/${data.productGroupCode}"
