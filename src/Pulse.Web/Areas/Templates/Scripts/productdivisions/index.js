...
$(document).ready(function () {
    var table = $('#productdivisionsTable').DataTable({
        "processing": true, "responsive": true,
        "serverSide": true,
        "ajax": {
            "url": getApiRootPath() + "/api/productDivisions/datatables",
            "type": "POST",
        ...
        "initComplete": function () {
            ...
            $tableContainer.find('.dt-paging').appendTo('.card-tools-productdivisionsTable-pagination');
            $tableContainer.find('.dt-search').appendTo('.card-tools-productdivisionsTable-filter');
            $tableContainer.find('.dt-length').appendTo('.card-tools-productdivisionsTable-length');
            $tableContainer.find('.dt-info').appendTo('.card-tools-productdivisionsTable-size');
          ...
          <div class="flex-fill pr-lg-3">
            <div class="d-flex flex-wrap align-items-center mb-1">
              ${status}
                ...
                <strong>${data.productDivisionName}</strong> (${data.productDivisionCode})
             
            </div>
            <div class="small text-muted mb-1">
            Description: ${data.productDivisionDescription ?? ''}
            </div>
            <div class="small text-muted mb-1">
...
<a title="Display" href="/templates/productDivisions/display/${data.productDivisionCode}"
                class="btn btn-outline-secondary rounded-circle d-inline-flex align-items-center justify-content-center p-0"
                style="width: 40px; height: 40px;">
    <i class="bi bi-eye"></i>
</a>
<a title="Config" href="/templates/productDivisions/edit/${data.productDivisionCode}"
